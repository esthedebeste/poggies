...
const rendered = await poggies.render(data);
...
writeFileSync(new URL("./output/poggies.html", import.meta.url), rendered);

const start = process.hrtime.bigint();
for (let i = 0; i < 100; i++) await poggies.render(data);
const end = process.hrtime.bigint();
const msAvg = Number((end - start) / BigInt(100)) / 1000000;
